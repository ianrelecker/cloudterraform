terraform {
  ...
  backend "azurerm" {
    ...
    key                  = "win11-d2s-vm.terraform.tfstate"
  }
}
...
variable "prefix"            { default = "win11-d2s" }
variable "location"          { default = "westus" }
variable "vm_size"           { default = "Standard_D2s_v6" }
variable "admin_username"    { default = "azureuser" }
variable "admin_password" {
  description = "Admin password for Windows 11 VM (must meet Azure complexity requirements)"
  type        = string
  sensitive   = true
}
variable "vnet_cidr"         { default = "10.71.0.0/16" }
variable "subnet_cidr"       { default = "10.71.1.0/24" }
...
# Initial setup (enable RDP, firewall, etc.)
