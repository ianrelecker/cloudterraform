terraform {
  ...
  backend "azurerm" {
    ...
    key                  = "ollama-t4-vmss.terraform.tfstate"
  }
}
...
# ---------------- Vars -----------------
variable "prefix"             { default = "ollama-t4" }
...
resource "azurerm_network_security_group" "nsg" {
  ...
  security_rule {
    ...
    # Restrict SSH to within the VNet (use Bastion/jumpbox)
    source_address_prefixes    = [var.vnet_cidr]
    destination_address_prefix = "*"
  }

  # Allow Ollama API within VNet only (private LB)
  security_rule {
    name                       = "ollama-vnet-in"
    priority                   = 120
    direction                  = "Inbound"
