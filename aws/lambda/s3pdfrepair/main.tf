terraform {
  ...
  backend "s3" {
    ...
    key    = "pdf-repair.terraform.tfstate"
    region = "us-west-2"
  }
}
...
locals {
  config = yamldecode(file("${path.module}/s3pdfrepair-config.yaml"))
}
...
output "lambda_function_name" {
  description = "Name of the PDF repair Lambda function"
  value       = aws_lambda_function.pdf_processor.function_name
}

output "lambda_function_arn" {
  description = "ARN of the PDF repair Lambda function"
...
output "download_example_url" {
  ...
  value       = local.config.create_api_gateway ? "https://${aws_api_gateway_rest_api.pdf_api[0].id}.execute-api.${data.aws_region.current.name}.amazonaws.com/prod/download/filename_repaired.pdf" : null
}
...
output "website_url" {
  description = "Public website URL for the PDF repairer"
